<template>
  <div>
    ...
    <div v-if="showAttributeForm" class="tw-flex tw-justify-end">
      <div
      ...
      >
        <span>{{ $__('back') }}</span>
        <span class="tw-px-1" />
        <i class="tw-block tw-w-8" v-html="initIcon('chevronLeft')"></i>
      </div>
    </div>
    <div class="tw-mt-8">
      <FormWrapper :form-data="state" @isValid="formIsValid = $event">
        <template v-if="!showAttributeForm">
        ...
        <div
          v-if="
            (attributesList.length && showAttributeForm) ||
            !attributesList.length
          "
          class="tw-mt-8 tw-text-center md:tw-text-start"
        >
          <button
        ...
        <div v-else class="tw-mt-8 tw-text-center md:tw-text-start">
          <button
            class="tw-w-full md:tw-w-max tw-bg-blue-600 hover:tw-bg-blue-500 tw-cursor-pointer tw-rounded tw-px-4 tw-py-2"
            @click="onShowAttributeForm"
          >
            {{ $__('next') }}
          </button>
        </div>
